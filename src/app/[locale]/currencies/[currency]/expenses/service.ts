...
export const createUserCurrencyExpenseGoal = async (
  ...
  const result = await expensesRepository.createGoal({
    ...dto,
    ownerId: session.user.id,
  });
...
export const getUserCurrencyExpenseGoals = async (
  ...
  const expenseGoals = await expensesRepository.findGoalsByUserCurrency(
    session.user.id,
    currency,
    fromTs,
  );
...
export const verifyExpenseGoalOwnership = async (
  ...
  const expenseGoal = await expensesRepository.findExpenseGoal(id);
...
export const destroySelfExpenseGoal = async (id: string) => {
  ...
  return expensesRepository.destroyExpenseGoal(id);
};
